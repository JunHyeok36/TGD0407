...
{
    [Serializable]
    public class _prototype_StateComponentData : _prototype_EntityComponentData, _prototype_IInteractable
    {
        [SerializeField]
        ...
        public Cysharp.Threading.Tasks.UniTask Interact(_prototype_EntityData caster, string interactionKey)
        {
            if (string.Equals(interactionKey, "TurnOn", StringComparison.OrdinalIgnoreCase))
            {
                SetState("Switch", "On");
            }
            else if (string.Equals(interactionKey, "TurnOff", StringComparison.OrdinalIgnoreCase))
            {
                SetState("Switch", "Off");
            }
            else if (interactionKey.StartsWith("SetState:", StringComparison.OrdinalIgnoreCase))
            {
                var parts = interactionKey.Substring(9).Split('=');
                if (parts.Length == 2)
                {
            ...
            {
                SetState(interactionKey, "Active");
            }
