using System;
using System.Collections.Generic;
using UnityEngine;

namespace TDG0407._prototype
{
    [Serializable]
    public sealed class _prototype_WithinCastRangeCondition : _prototype_Condition
    {
        [UnityEngine.SerializeReference, SubclassSelector]
        public _prototype_ICastRangeSelector castRange = new _prototype_AroundRectCastSelector();

        public override _prototype_ConditionResult Evaluate(
            _prototype_ConditionContext context)
        {
            if (castRange == null)
                return _prototype_ConditionResult.Fail("No interaction range is configured.");

            List<_prototype_Point> points = castRange.GetValidCastPoints(context.Actor.point);
            return points.Contains(context.Target.point)
                ? _prototype_ConditionResult.Success()
                : _prototype_ConditionResult.Fail("The interactable is out of range.");
        }
    }
}
